...
sync() {
    ...
    rsync -r --copy-links --exclude-from=$EXCLUDE_FILE "${cdir}"/../campfire/desk/* $DESK_DIR/
    rsync -r --copy-links --exclude-from=$EXCLUDE_FILE "${cdir}"/../icepond/* $DESK_DIR/
    rsync -r --copy-links --exclude-from=$EXCLUDE_FILE "${cdir}"/../rtcswitchboard/* $DESK_DIR/
    if [ "$GLOBBER" ]; then
        rsync -r --copy-links "${cdir}"/../dist/* $DESK_DIR/call
    fi
}
...
if [ -z "$WATCH_MODE" ]; then
    echo "Installing %call to ${DESK_DIR}"
    ...
    if [ "$GLOBBER" ]; then
        lensd '-garden!make-glob %globber /call'
        GLOBS="$PIER/.urb/put"
